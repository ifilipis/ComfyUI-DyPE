    ...
    if enable_dype and should_patch_schedule:
        try:
            if isinstance(m.model.model_sampling, model_sampling.ModelSamplingFlux) or is_qwen or is_z_image:
                base_latent_h = base_patch_grid[0] * patch_size
                base_latent_w = base_patch_grid[1] * patch_size

                latent_scale_h = base_resolution / base_latent_h if base_latent_h > 0 else 1.0
                latent_scale_w = base_resolution / base_latent_w if base_latent_w > 0 else 1.0

                latent_h = math.ceil(height / latent_scale_h)
                latent_w = math.ceil(width / latent_scale_w)
