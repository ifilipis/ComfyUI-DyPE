    ...
    if is_zimage:
        ...
        def dype_patchify_and_embed(self, x, cap_feats, cap_mask, t, num_tokens, transformer_options={}):
            ...
            if rope_embedder is not None:
                ...
                if t is not None and isinstance(t, torch.Tensor) and t.numel() > 0 and sigma_max > 0:
                    current_sigma = t.flatten()[0].item()
                    t_norm = min(max(current_sigma / sigma_max, 0.0), 1.0)

                    if dype_exponent is not None:
                        dype_blend_factor = math.pow(t_norm, dype_exponent)
                    else:
                        dype_blend_factor = t_norm
