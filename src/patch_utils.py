...
def apply_dype_to_model(model: ModelPatcher, model_type: str, width: int, height: int, method: str, yarn_alt_scaling: bool, enable_dype: bool, dype_scale: float, dype_exponent: float, base_shift: float, max_shift: float, base_resolution: int = 1024, dype_start_sigma: float = 1.0) -> ModelPatcher:
    ...
    if is_z_image and enable_dype:
        ...
        if original_patchify_and_embed is not None:
            ...
            base_patches = (base_resolution // 8) // 2
            m.model.diffusion_model._dype_base_hw = (base_patches, base_patches)
            ...
            def dype_patchify_and_embed(self, x, cap_feats, cap_mask, t, num_tokens, transformer_options={}):
                ...
                if rope_embedder is not None:
                    dype_start_sigma_local = getattr(rope_embedder, "dype_start_sigma", None)
                    dype_exponent_local = getattr(rope_embedder, "dype_exponent", None)
                    current_timestep = getattr(rope_embedder, "current_timestep", None)

                    if all(value is not None for value in (dype_start_sigma_local, dype_exponent_local, current_timestep)) and dype_start_sigma_local > 0:
                        if current_timestep > dype_start_sigma_local:
                            t_norm = 1.0
                        else:
                            t_norm = current_timestep / dype_start_sigma_local

                        dype_blend_factor = math.pow(t_norm, dype_exponent_local)
