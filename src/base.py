...
class DyPEBasePosEmbed(nn.Module):
    ...
    def __init__(self, theta: int, axes_dim: list[int], method: str = 'yarn', yarn_alt_scaling: bool = False, dype: bool = True, dype_scale: float = 2.0, dype_exponent: float = 2.0, base_resolution: int = 1024, dype_start_sigma: float = 1.0, base_patches: int | list[int] | tuple[int, ...] | None = None):
        ...
        if base_patches is None:
            self.base_patches = default_base
            self.base_patches_per_axis = [default_base]
        elif isinstance(base_patches, (list, tuple)):
            self.base_patches_per_axis = [int(x) for x in base_patches]
            self.base_patches = max(self.base_patches_per_axis)
        else:
            self.base_patches = int(base_patches)
            self.base_patches_per_axis = [self.base_patches]
