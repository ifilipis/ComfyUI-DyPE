...
class DyPEBasePosEmbed(nn.Module):
    ...
    def __init__(self, theta: int, axes_dim: list[int], method: str = 'yarn', yarn_alt_scaling: bool = False, dype: bool = True, dype_scale: float = 2.0, dype_exponent: float = 2.0, base_resolution: int = 1024, dype_start_sigma: float = 1.0, base_patches: int | None = None, base_hw: tuple[int, int] | None = None):
        ...
        elif self.base_hw is not None:
            self.base_patches = max(self.base_hw)
        else:
            self.base_patches = (self.base_resolution // 8) // 2

    def _axis_base_patches(self, axis_index: int) -> int:
        if self.base_hw is not None and axis_index in (1, 2):
            if axis_index == 1:
                return max(1, int(self.base_hw[0]))
            if axis_index == 2:
                return max(1, int(self.base_hw[1]))
        return max(1, int(self.base_patches))
    ...
    def _calc_vision_yarn_components(self, pos: torch.Tensor, freqs_dtype: torch.dtype):
        ...
        if pos.shape[-1] >= 3:
            ...
            base_for_global = max(self._axis_base_patches(1), self._axis_base_patches(2))
        else:
            max_current_patches = int(pos.max().item() - pos.min().item() + 1)
            base_for_global = self._axis_base_patches(0)

        scale_global = max(1.0, max_current_patches / base_for_global)
        ...
        for i in range(n_axes):
            ...
            base_patches_for_axis = self._axis_base_patches(i)
            ...
            if i > 0:
                scale_local = max(1.0, current_patches / base_patches_for_axis)
                ...
                if scale_global > 1.0:
                    cos, sin = get_1d_dype_yarn_pos_embed(
                        **common_kwargs,
                        ori_max_pe_len=base_patches_for_axis,
                        **dype_kwargs
                    )
                else:
    ...
    def _calc_yarn_components(self, pos: torch.Tensor, freqs_dtype: torch.dtype):
        ...
        needs_extrapolation = (max_current_patches > base_for_global)
        ...
        if needs_extrapolation and use_anisotropic:
            for i in range(n_axes):
                ...
                if i > 0 and current_patches_on_axis > base_patches_for_axis:
                    max_pe_len = torch.tensor(current_patches_on_axis, dtype=freqs_dtype, device=pos.device)
                    cos, sin = get_1d_yarn_pos_embed(**common_kwargs, max_pe_len=max_pe_len, ori_max_pe_len=base_patches_for_axis, **dype_kwargs, use_aggressive_mscale=True)
                else:
                    cos, sin = get_1d_ntk_pos_embed(**common_kwargs, ntk_factor=1.0)
            ...
            if needs_extrapolation:
                ...
                base_for_spatial = max(self._axis_base_patches(1), self._axis_base_patches(2))
                ...
                cos_full_spatial, sin_full_spatial = get_1d_yarn_pos_embed(
                    **common_kwargs_spatial, pos=square_pos, max_pe_len=max_pe_len, ori_max_pe_len=base_for_spatial, **dype_kwargs, use_aggressive_mscale=False
                )
