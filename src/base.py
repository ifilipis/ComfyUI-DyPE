...
class DyPEBasePosEmbed(nn.Module):
    ...
    """
    def __init__(self, theta: int, axes_dim: list[int], method: str = 'yarn', yarn_alt_scaling: bool = False, dype: bool = True, dype_scale: float = 2.0, dype_exponent: float = 2.0, base_resolution: int = 1024, dype_start_sigma: float = 1.0, base_patches: int | None = None, base_hw_tokens: Optional[Tuple[int, int]] = None, target_hw_tokens: Optional[Tuple[int, int]] = None):
        super().__init__()
        self.theta = theta
        ...
        # Optional per-axis target token counts for anisotropic grids (e.g., Z-Image densification).
        self.target_hw_tokens: Optional[Tuple[int, int]] = target_hw_tokens
    ...
    def _get_target_span(self, axis_index: int, measured_span: float) -> float:
        if self.target_hw_tokens is not None and axis_index > 0:
            idx = min(axis_index - 1, len(self.target_hw_tokens) - 1)
            return float(self.target_hw_tokens[idx])
    ...
    def _calc_vision_yarn_components(self, pos: torch.Tensor, freqs_dtype: torch.dtype):
        ...
        if pos.shape[-1] >= 3:
            ...
            global_scales.append(self._get_target_span(1, h_span) / max(1.0, self._get_base_for_axis(1)))
            global_scales.append(self._get_target_span(2, w_span) / max(1.0, self._get_base_for_axis(2)))
        else:
            span = float(pos.max().item() - pos.min().item() + 1.0)
            global_scales.append(self._get_target_span(1, span) / max(1.0, self._get_base_for_axis(1)))
        ...
        for i in range(n_axes):
            ...
            current_patches = self._get_target_span(i, float(axis_pos.max().item() - axis_pos.min().item() + 1.0))
    ...
    def _calc_yarn_components(self, pos: torch.Tensor, freqs_dtype: torch.dtype):
        ...
        if pos.shape[-1] >= 3:
            h_span = float(pos[..., 1].max().item() - pos[..., 1].min().item() + 1.0)
            w_span = float(pos[..., 2].max().item() - pos[..., 2].min().item() + 1.0)
            axis_spans.append((1, self._get_target_span(1, h_span)))
            axis_spans.append((2, self._get_target_span(2, w_span)))
        else:
            span = float(pos.max().item() - pos.min().item() + 1.0)
            axis_spans.append((1, self._get_target_span(1, span)))
        ...
        if needs_extrapolation and use_anisotropic:
            for i in range(n_axes):
                ...
                current_patches_on_axis = self._get_target_span(i, float(axis_pos.max().item() - axis_pos.min().item() + 1.0))
                base_for_axis = self._get_base_for_axis(i)
